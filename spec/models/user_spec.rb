...
RSpec.describe User, type: :model do
  it 'validates user' do
    user = User.new(username: 'user_one', password: '123456')
  ...
  it "if length of name is < 3, doesn't validate user" do
    user = User.new(username: 'aa', password: '123456')
    expect(user.valid?).to be false
  end

  it "doesn't validate user if length of name is > 16" do
    user = User.new(username: 'a' * 17, password: '123456')
    expect(user.valid?).to be false
  end
